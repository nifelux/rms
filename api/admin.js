...
async function getUserDetail(req, res) {
  ...
  const directReferrals = referrals.data || [];
  let activeReferralIds = [];
  if (directReferrals.length > 0) {
    const { data: approvedReferralDeposits } = await supabaseAdmin
      .from('deposits')
      .select('user_id')
      .eq('status', 'approved')
      .in('user_id', directReferrals.map(referral => referral.id));
    activeReferralIds = [...new Set((approvedReferralDeposits || []).map(deposit => deposit.user_id))];
  }
  const enrichedReferrals = directReferrals.map(referral => ({
    ...referral,
    is_active: activeReferralIds.includes(referral.id)
  }));
