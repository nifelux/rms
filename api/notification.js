...
// RMS FIX: Replace missing Core function imports with direct implementations
  ...
  try {
    ...
    if (update.message && update.message.text) {
      ...
      switch (command) {
        ...
        // RMS FIX: Removed /kyc command (KYC removed from RMS)
        ...
        // RMS FIX: Changed from /tiers to support both old and new naming
        case '/tiers': 
        case '/rmstiers': return await handleTiers(chatId, res);
        ...
        // RMS FIX: Removed /purgeproofs command (withdrawal proofs removed from RMS)
...
async function handleCallbackQuery(callbackQuery, res) {
  ...
  const actions = {
    ...
    reject_withdrawal: async () => {
      const { error } = await supabaseAdmin.from('withdrawals').update({ status: 'rejected', updated_at: new Date() }).eq('id', id);
      if (error) return { ok: false, error: error.message };
      
      // Update transaction
      await supabaseAdmin.from('transactions').update({ status: 'rejected' }).eq('reference', `wd_${id}`);
      return { ok: true, message: 'Withdrawal rejected' };
    },
    // RMS FIX: Removed approve_kyc and reject_kyc (KYC removed)
  };
...
// RMS FIX: Removed updateKyc function (KYC removed)

async function handleKyc(chatId, res) {
  // RMS FIX: KYC removed, show message instead
  await sendToTelegram(chatId, '<b>KYC Feature:</b>\nKYC verification has been disabled in RMS.');
  return res.status(200).end();
}

async function handleProducts(chatId, res) { 
  const { data, error } = await supabaseAdmin.from('products').select('id,name,min_invest,max_invest,daily_roi_percent,duration_days,is_locked').order('created_at',{ascending:false}).limit(20); 
  if(error) throw error; 
  const msg=(data||[]).map(p=>`<b>${tgEscape(p.name)}</b> · ${p.is_locked?'🔒 locked':'✅ open'}\nID: <code>${p.id}</code>\nRange: ₦${Number(p.min_invest||0).toLocaleString()}–${Number(p.max_invest||0).toLocaleString()} · ROI ${p.daily_roi_percent||0}% · ${p.duration_days||0} days`).join('\n\n') || 'No products.'; 
  await sendToTelegram(chatId,`<b>Products</b>\n${msg}`); 
  return res.status(200).end(); 
}

async function handleLockProduct(chatId, text, res) { 
  const [id, value] = text.split(/\s+/).slice(1); 
  if(!id || !['on','off','lock','unlock'].includes((value||'').toLowerCase())) { 
    await sendToTelegram(chatId,'Usage: /lockproduct <id> <on|off>'); 
    return res.status(200).end(); 
  } 
  const is_locked=['on','lock'].includes(value.toLowerCase()); 
  const {error}=await supabaseAdmin.from('products').update({is_locked}).eq('id',id); 
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:`✅ Product ${is_locked?'locked':'unlocked'}`); 
  return res.status(200).end(); 
}

async function handleCreateProduct(chatId, text, res) { 
  const parts=text.slice(text.indexOf(' ')+1).split('|').map(x=>x.trim()); 
  if(parts.length<6){
    await sendToTelegram(chatId,'Usage: /createproduct name|description|minimum|max|roi_percent|duration_days|daily_income|max_purchases|category');
    return res.status(200).end();
  } 
  const [name,description,min,max,roi,duration,daily,maxPurchases,category]=parts; 
  const {data,error}=await supabaseAdmin.from('products').insert({
    name,
    description,
    min_invest:Number(min),
    max_invest:Number(max)||null,
    daily_roi_percent:Number(roi),
    duration_days:Number(duration),
    daily_income_amount:Number(daily)||null,
    max_purchases_per_user:Number(maxPurchases)||null,
    category:category||'investment'
  }).select().single(); 
  await sendToTelegram(chatId,error?`️ ${tgEscape(error.message)}`:`✅ Product created: <b>${tgEscape(data.name)}</b>`); 
  return res.status(200).end(); 
}
...
async function handleUpdateProduct(chatId, text, res) { 
  const parts=text.slice(text.indexOf(' ')+1).split('|').map(x=>x.trim()); 
  if(parts.length<7){
    await sendToTelegram(chatId,'Usage: /updateproduct id|name|description|min|max|roi_percent|duration_days|daily_income|max_purchases|category');
    return res.status(200).end();
  } 
  const [id,name,description,min,max,roi,duration,daily,maxPurchases,category]=parts; 
  ...
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:'✅ Product updated'); 
  return res.status(200).end(); 
}
...
async function handleDeleteProduct(chatId, text, res) { 
  ...
  if(!id){
    await sendToTelegram(chatId,'Usage: /deleteproduct <id>');
    return res.status(200).end();
  } 
  const {error}=await supabaseAdmin.from('products').delete().eq('id',id); 
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:'✅ Product deleted'); 
  return res.status(200).end(); 
}

async function handleRunIncome(chatId, res) { 
  const today=new Date().toISOString().slice(0,10); 
  const {data:due,error}=await supabaseAdmin.from('investments').select('*').eq('status','active').or(`last_income_date.is.null,last_income_date.lt.${today}`); 
  if(error) throw error; 
  let paid=0,completed=0; 
  for(const inv of due||[]){
    const days=Number(inv.days_elapsed)+1;
    const total=Number(inv.total_income)+Number(inv.daily_income);
    const done=days>=Number(inv.duration_days);
    const tx=await supabaseAdmin.from('transactions').insert({
      user_id:inv.user_id,
      type:'earning',
      amount:Number(inv.daily_income),
      status:'approved',
      reference:`inv_income_${inv.id}_day${days}`
    });
    if(tx.error&&!tx.error.message.includes('duplicate'))continue;
  ...
  await supabaseAdmin.from('cron_logs').insert({
    ...
    status:'success',
    details:{paidCount:paid,completedCount:completed,triggeredBy:'telegram'}
  }); 
  await sendToTelegram(chatId,`✅ Daily income processed\nPaid: ${paid}\nCompleted: ${completed}`); 
  return res.status(200).end(); 
}

async function handleCron(chatId, res) { 
  const {data,error}=await supabaseAdmin.from('cron_logs').select('*').order('created_at',{ascending:false}).limit(5); 
  if(error) throw error; 
  await sendToTelegram(chatId,`<b>Recent cron runs</b>\n${(data||[]).map(x=>`${tgEscape(x.job_name)} · ${tgEscape(x.status)} · ${new Date(x.created_at).toLocaleString()}`).join('\n')||'None'}`); 
  return res.status(200).end(); 
}

async function handleHealth(chatId, res) { 
  const {data,error}=await supabaseAdmin.from('cron_logs').select('*').order('created_at',{ascending:false}).limit(1); 
  if(error) throw error; 
  await sendToTelegram(chatId,`<b>System health</b>\nDatabase: ✅\nLast cron: ${data?.[0]?`${tgEscape(data[0].job_name)} · ${tgEscape(data[0].status)}`:'None'}`); 
  return res.status(200).end(); 
}

async function handleAdmins(chatId, res) { 
  const {data,error}=await supabaseAdmin.from('profiles').select('id,email,full_name').eq('is_admin',true); 
  if(error) throw error; 
  await sendToTelegram(chatId,`<b>Admins</b>\n${(data||[]).map(a=>`${tgEscape(a.full_name||'N/A')} · ${tgEscape(a.email)}\n<code>${a.id}</code>`).join('\n\n')||'None'}`); 
  return res.status(200).end(); 
}

async function handleRoleChange(chatId, text, res, promote) { 
  const id=text.split(/\s+/)[1]; 
  if(!id){
    await sendToTelegram(chatId,`Usage: /${promote?'promote':'demote'} <user_id>`);
    return res.status(200).end();
  } 
  const {error}=await supabaseAdmin.from('profiles').update({is_admin:promote}).eq('id',id); 
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:`✅ ${promote?'Promoted to':'Removed from'} admin: <code>${id}</code>`); 
  return res.status(200).end(); 
}
...
async function handleTiers(chatId, res) { 
  // RMS FIX: Try rms_tiers first, fallback to task_tiers for backwards compatibility
  let {data,error} = await supabaseAdmin.from('rms_tiers').select('*').order('upgrade_cost', {ascending: true});
  if (error) {
    // Fallback to old task_tiers table
    const {data:oldData,error:oldError} = await supabaseAdmin.from('task_tiers').select('*').order('sort_order');
    if(oldError) throw oldError;
    data = oldData;
  }
  
  await sendToTelegram(chatId,`<b>Task tiers</b>\n${(data||[]).map(t=>`${tgEscape(t.name || t.tier)} · ID <code>${t.id}</code>\nPrice ₦${t.price || t.upgrade_cost} · ${t.tasks_per_day || t.daily_boxes}/day · ₦${t.pay_per_task || t.box_earning}/task`).join('\n\n')||'None'}`); 
  return res.status(200).end(); 
}
...
async function handleTierUpdate(chatId, text, res) { 
  const parts=text.slice(text.indexOf(' ')+1).split('|').map(x=>x.trim()); 
  if(parts.length!==4){
    await sendToTelegram(chatId,'Usage: /tier <id>|<price>|<tasks_per_day>|<pay_per_task>');
    return res.status(200).end();
  } 
  const [id,price,tasks,pay]=parts; 
  // RMS FIX: Try rms_tiers first, fallback to task_tiers
  let {error}=await supabaseAdmin.from('rms_tiers').update({
    upgrade_cost:Number(price),
    daily_boxes:Number(tasks),
    box_earning:Number(pay)
  }).eq('id',id);
  
  if (error) {
    // Fallback to old task_tiers table
    error = (await supabaseAdmin.from('task_tiers').update({
      price:Number(price),
      tasks_per_day:Number(tasks),
      pay_per_task:Number(pay)
    }).eq('id',id)).error;
  }
  
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:`✅ Tier updated: <code>${id}</code>`); 
  return res.status(200).end(); 
}
...
async function handleVip(chatId, text, res) { 
  const parts=text.slice(text.indexOf(' ')+1).split('|').map(x=>x.trim()); 
  if(parts.length<5){
    await sendToTelegram(chatId,'Usage: /vip <level>|<min_deposit>|<min_investments>|<min_referrals>|<daily_bonus_percent>');
    return res.status(200).end();
  } 
  const [level,minDeposit,minInvestments,minReferrals,bonus]=parts; 
  const {data,error}=await supabaseAdmin.from('vip_levels').insert({
    level,
    min_deposit:Number(minDeposit)||0,
    min_investments:Number(minInvestments)||0,
    min_referrals:Number(minReferrals)||0,
    daily_bonus_percent:Number(bonus)||0
  }).select().single(); 
  await sendToTelegram(chatId,error?`⚠️ ${tgEscape(error.message)}`:`✅ VIP level created: ${tgEscape(data.level)}`); 
  return res.status(200).end(); 
}

async function handleVipLevels(chatId, res) { 
  const {data,error}=await supabaseAdmin.from('vip_levels').select('*').order('level'); 
  if(error) throw error; 
  await sendToTelegram(chatId,`<b>VIP levels</b>\n${(data||[]).map(v=>`${tgEscape(v.level)} · Deposit ₦${v.min_deposit||0} · Investments ₦${v.min_investments||0} · Referrals ${v.min_referrals||0} · Bonus ${v.daily_bonus_percent||0}%`).join('\n')||'None'}`); 
  return res.status(200).end(); 
}
...
async function handleNotifications(chatId, res) { 
  const {data,error}=await supabaseAdmin.from('notifications').select('title,body,created_at,profiles(email)').order('created_at',{ascending:false}).limit(10); 
  if(error) throw error; 
  await sendToTelegram(chatId,`<b>Recent notifications</b>\n${(data||[]).map(n=>`${tgEscape(n.title)} · ${tgEscape(n.profiles?.email||'All users')}\n${tgEscape(n.body||'')}`).join('\n\n')||'None'}`); 
  return res.status(200).end(); 
}
...
async function handlePurgeProofs(chatId, res) {
  // RMS FIX: Withdrawal proofs removed, show message
  await sendToTelegram(chatId, 'ℹ️ <b>Purge Proofs:</b>\nWithdrawal proof feature has been disabled in RMS.');
  res.status(200).end();
}
...
async function handleGiftCode(chatId, text, res) {
  ...
  const customCode = parts[2] || generateRandomGiftCode();
  ...
  const result = await createGiftCodeCore(customCode, amount, maxUses);
...
// RMS FIX: Implement these Core functions directly
...
async function createGiftCodeCore(code, amount, maxUses) {
  try {
...
<b>Removed in RMS:</b>
/kyc - KYC verification disabled
/purgeproofs - Withdrawal proofs disabled`;
  await sendToTelegram(chatId, msg);
  res.status(200).end();
}
