    ...
    if (tier === 'M0') {
      ...
      if (hoursSinceM0 >= 24) {
        return res.status(200).json({
          ...
          can_open: false,
          m0_expired: true,
          message: 'Your free M0 trial has expired. Upgrade to continue.'
        });
      }
    }
...
async function openMysteryBox(req, res) {
  try {
    ...
    if (!tier || tier === 'newbie') {
      return res.status(400).json({ error: 'You must have an active tier to open boxes' });
    }
    ...
    if (tier === 'M0') {
      ...
      if (hoursSinceM0 >= 24) {
        return res.status(400).json({ error: 'Your M0 trial has expired. Please upgrade.' });
      }
    }
    ...
    if (boxesOpened >= tierInfo.daily_boxes) {
      return res.status(400).json({ error: `Daily limit reached (${tierInfo.daily_boxes} boxes)` });
    }
