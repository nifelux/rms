/**
 * TargetGrowths Payment Gateway Integration
 * ES Module Version
 */
...
function parseWebhookBody(rawBody) {
  // Handle case where body is already a parsed object (Vercel auto-parses JSON)
  if (rawBody && typeof rawBody === 'object' && !Buffer.isBuffer(rawBody)) {
    // Check if it has data[amount] format - convert to nested structure
    const payload = { ...rawBody };
    const nestedData = {};
    
    // Convert data[xxx] keys to nested data object
...
function webhookSignatureCheck(payload) {
  const { secretKey, publicKey } = getCredentials();
  const data = webhookData(payload);
  ...
  console.log('[TG-SIG-DEBUG] === COMPREHENSIVE SIGNATURE TEST ===');
  console.log('[TG-SIG-DEBUG] Signature from TG:', signature);
  console.log('[TG-SIG-DEBUG] Identifier:', identifier);
  console.log('[TG-SIG-DEBUG] Amount Raw:', amountRaw);
  console.log('[TG-SIG-DEBUG] Status:', status);
  console.log('[TG-SIG-DEBUG] Secret Key:', secretKey);
  console.log('[TG-SIG-DEBUG] Public Key:', publicKey);
  
  if (!signature || !identifier || amountRaw === null) {
    console.log('[TG-SIG-DEBUG] ❌ Missing required fields');
    return { valid: false, amountRaw, identifier, matched: null };
  }

  // Try BOTH keys (secret and public)
  const keysToTry = [
    { key: secretKey, name: 'Secret Key' },
    { key: publicKey, name: 'Public Key' },
  ];
  
  // Try BOTH hash algorithms
  const algorithms = ['sha256', 'md5', 'sha1', 'sha512'];
  
  // Try different data combinations
  const dataFormats = [
    `${amountRaw}${identifier}`,
    `${identifier}${amountRaw}`,
    `${amount}${identifier}`,
    `${identifier}${amount}`,
    `${amountRaw}|${identifier}`,
    `${identifier}|${amountRaw}`,
    ...
    `${amountRaw}-${identifier}`,
    `${identifier}-${amountRaw}`,
    `${amountRaw}${identifier}${status}`,
    `${identifier}${amountRaw}${status}`,
    `${status}${amountRaw}${identifier}`,
    `${status}${identifier}${amountRaw}`,
    `${amountRaw}${identifier}${secretKey}`,
    `${identifier}${amountRaw}${secretKey}`,
    `${secretKey}${amountRaw}${identifier}`,
    `${secretKey}${identifier}${amountRaw}`,
    `${amountRaw}${identifier}${publicKey}`,
    `${identifier}${amountRaw}${publicKey}`,
  ];
  ...
  for (const keyObj of keysToTry) {
    for (const algo of algorithms) {
      for (const format of dataFormats) {
        try {
          const hash = crypto.createHmac(algo, keyObj.key)
            .update(format)
            .digest("hex");
          ...
          if (hashUpper === signature || hashLower === signature) {
            console.log(`[TG-SIG-DEBUG] ✅ MATCH FOUND!`);
            console.log(`[TG-SIG-DEBUG] Algorithm: ${algo}`);
            console.log(`[TG-SIG-DEBUG] Key: ${keyObj.name}`);
            console.log(`[TG-SIG-DEBUG] Format: "${format}"`);
            console.log(`[TG-SIG-DEBUG] Hash: ${hashUpper}`);
            return { valid: true, amountRaw, identifier, matched: amountRaw };
          }
        } catch (e) {
  ...
  // Also try without HMAC (just plain hash)
  console.log('[TG-SIG-DEBUG] Trying plain hash (no HMAC)...');
  for (const algo of algorithms) {
    for (const format of dataFormats.slice(0, 10)) {
      try {
        const hash = crypto.createHash(algo).update(format).digest("hex");
        if (hash.toUpperCase() === signature || hash.toLowerCase() === signature) {
          console.log(`[TG-SIG-DEBUG] ✅ PLAIN HASH MATCH!`);
          console.log(`[TG-SIG-DEBUG] Algorithm: ${algo}`);
          console.log(`[TG-SIG-DEBUG] Format: "${format}"`);
          return { valid: true, amountRaw, identifier, matched: amountRaw };
        }
  ...
  console.log('[TG-SIG-DEBUG] ❌ NO MATCH - Tried all combinations');
  console.log('[TG-SIG-DEBUG] Please contact Target Growth support for signature format documentation');
  return { valid: false, amountRaw, identifier, matched: null };
}
