 ...
 * Fixed Version - Uses precise SHA256 signature like Schweppes
 */
...
// ==========================================
// WEBHOOK SIGNATURE VERIFICATION (FIXED - Uses Schweppes approach)
// ==========================================
function webhookSignatureCheck(payload) {
  ...
  console.log('[TG-SIG-CHECK] Signature:', signature);
  console.log('[TG-SIG-CHECK] Identifier:', identifier);
  console.log('[TG-SIG-CHECK] Amount:', amountRaw);
  console.log('[TG-SIG-CHECK] Charge:', chargeRaw);
  
  if (!signature || !identifier || amountRaw === null || !Number.isFinite(amount)) {
    console.log('[TG-SIG-CHECK] ❌ Missing required fields');
    return { valid: false, amountRaw, identifier, chargeRaw, matched: null };
  }

  // Build amount candidates (including charge-aware amounts like Schweppes)
  const candidates = new Set(amountFormats(amountRaw));
  
  // Add charge-aware candidates (gross amount, net amount, etc.)
  if (Number.isFinite(charge)) {
    for (const value of [amount + charge, amount - charge, charge]) {
      for (const formatted of amountFormats(value)) {
  ...
  // Try each candidate with SHA256 HMAC (the correct algorithm)
  for (const value of candidates) {
    ...
    if (expected.length === provided.length && 
      ...
      console.log(`[TG-SIG-CHECK] ✅ Signature matched! Amount: ${value}`);
      return { valid: true, amountRaw, identifier, chargeRaw, matched: value };
    }
  }
  
  console.log('[TG-SIG-CHECK] ❌ No signature match');
  return { valid: false, amountRaw, identifier, chargeRaw, matched: null };
}
