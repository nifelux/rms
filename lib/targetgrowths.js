...
function webhookSignatureCheck(payload) {
  ...
  console.log('[TG-SIG-DEBUG] Full Payload:', JSON.stringify(payload, null, 2));
  ...
  const formats = [
    `${amountRaw}${identifier}`,           // Amount + Identifier
    `${identifier}${amountRaw}`,           // Identifier + Amount
    `${amount}${identifier}`,              // Amount (number) + Identifier
    `${identifier}${amount}`,              // Identifier + Amount (number)
    `${amountRaw}${identifier}${secretKey}`, // With key appended
  ...
  for (const format of formats) {
    ...
    console.log(`[TG-SIG-DEBUG] Format "${format}" → Hash: ${hash}`);
    
    if (hash === signature) {
      console.log(`[TG-SIG-DEBUG] ✅ MATCH! Using format: ${format}`);
      return { valid: true, amountRaw, identifier, matched: amountRaw };
    }
  }
  
  console.log('[TG-SIG-DEBUG] ❌ NO MATCH - None of the formats worked');
  return { valid: false, amountRaw, identifier, matched: null };
}
